...
import { useAllMeetings, useCommittees } from "~/hooks/useApi";
...
const TAG_STYLES = [
  { tagColor: "border-l-[#2563EB]", tagBg: "bg-blue-50 text-blue-700 dark:bg-blue-950/40 dark:text-blue-300" },
  { tagColor: "border-l-[#7F56D9]", tagBg: "bg-purple-50 text-purple-700 dark:bg-purple-950/40 dark:text-purple-300" },
  { tagColor: "border-l-[#EA580C]", tagBg: "bg-orange-50 text-orange-700 dark:bg-orange-950/40 dark:text-orange-300" },
];
...
export const UpcomingMeetingsWidget: React.FC<UpcomingMeetingsWidgetProps> = ({
  ...
  const { data: rawMeetings = [] } = useAllMeetings();
  const { data: committees = [] } = useCommittees();

  const meetings = useMemo(() => {
    if (!rawMeetings || rawMeetings.length === 0) return [];
    
    // Filter upcoming or sort by latest
    const upcoming = rawMeetings.filter((m: any) => m.type === "Upcoming" || !m.type || m.type === "Pending");
    const list = upcoming.length > 0 ? upcoming : rawMeetings;

    return list.slice(0, 4).map((m: any, idx: number) => {
      const committeeObj = Array.isArray(committees) ? committees.find((c: any) => c.id === m.committeeId) : null;
      const committeeName = committeeObj ? committeeObj.name.replace(/committee/i, "").trim() : "IEEE";
      ...
      return {
        ...
        tagBg: style.tagBg,
        location: m.description ? (m.description.length > 20 ? `${m.description.substring(0, 18)}...` : m.description) : "Main Hall",
        date: dateStr,
        time: m.recap || "10:00 AM",
      };
    });
  }, [rawMeetings, committees]);
